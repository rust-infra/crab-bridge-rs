...
pub struct SetupOptions {
    ...
    pub bind_addr: SocketAddr,
    /// When true, write multi-provider `crabbridge.toml` (first provider in a batch only).
...
pub async fn run_setup(opts: SetupOptions) -> Result<()> {
    ...
    if bridge_config_created && let Some(path) = &bridge_config_path {
        info!(path = %path.display(), "wrote bridge config");
    }
...
pub fn merge_codex_config(
    ...
    info!(
        ...
        set_active,
        "Updated Codex config"
    );
    Ok(())
}

/// Set the active Codex provider without touching bridge config or model catalog.
pub fn set_codex_active_provider(bind_addr: SocketAddr, slug: &str) -> Result<()> {
    let codex_home =
        codex_home_dir().context("Cannot resolve Codex home directory (HOME/CODEX_HOME unset)")?;
    let codex_config_path = codex_home.join("config.toml");
    let codex_provider_name = ProviderKind::codex_provider_name(slug);
    let catalog_path = catalog_path_for_slug(slug);
    let bridge_base_url = format!("http://{bind_addr}/{slug}/v1");
    let kind =
        ProviderKind::from_route(slug).with_context(|| format!("unknown provider slug: {slug}"))?;
    ...
    info!(slug, "Set Codex active provider");
    Ok(())
}
...
pub async fn collect_setup_check(opts: SetupCheckOptions) -> SetupCheckReport {
    ...
    match model_provider {
        ...
        None => push_check(
            ...
            "model_provider",
            CheckStatus::Fail,
            "not set — run Setup from the CrabBridge menu".into(),
        ),
    }
        ...
        match provider_table {
            ...
            None => {
                push_check(
                    ...
                    CheckStatus::Fail,
                    "section missing — run Setup from the CrabBridge menu".into(),
                );
            }
        }
...
fn check_bridge_config_file(checks: &mut Vec<ConfigCheck>, path: &Path) {
    if !path.exists() {
        ...
        push_check(
            ...
            CheckStatus::Warn,
            format!(
                "{} not found (optional — run Setup from the CrabBridge menu to generate)",
                path.display()
            ),
        );
        return;
